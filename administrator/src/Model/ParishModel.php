 ...
 * @since  1.6
 */
class ParishModel extends AdminModel
{
	/**
	 ...
	 * @since   3.2
	 */
	public $typeAlias = 'com_xws_property.parish';
	 ...
	 * @since    1.6
	 */
	public function getTable($type = 'Parish', $prefix = 'Administrator', $config = array())
	{
	...
	public function getForm($data = array(), $loadData = true)
	{
            ...
            // Get the form.
            $form = $this->loadForm(
                    'com_xws_property.parish', 'parish',
                    array('control' => 'jform',
                            'load_data' => $loadData
                    )
            );
	...
	protected function loadFormData()
	{
		...
		$data = Factory::getApplication()->getUserState('com_xws_property.edit.parish.data', array());
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = Factory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__xws_property_parishes');
				$max             = $db->loadResult();
